...
# ============== Optimize the trigger using data from the data folder ==============
...
# ==========================================
class Config:
    DATA_DIR = "./data"                 # Path to benign data
    ...
    INIT_TRIGGER = ["your", "init", "trigger"]      # Initial trigger text

    # Loss weights
    LAMBDA_CPT = 1.8                  # Cluster compactness loss weight
    LAMBDA_UNI = 0.8                   # Distribution uniformity loss weight
...
def load_and_cluster_benign(attacker):
    ...
    2. Use sentence embeddings and KMeans to obtain N centers (strict L_uni constraint)
    ...
    # Iterate over all json files and add each qualified text (text length requirement) into corpus (list)
    ...
    for file in json_files:
        try:
            with open(file, 'r', encoding='utf-8') as f:
                content = json.load(f)
                # Normalize JSON into list / single-item formats
    ...
    # 1. Encode all benign texts as sentence vectors
    ...
    # 3. Convert to GPU tensor, shape [N, vector_dim]
...
class AttackModel:
    def __init__(self, device):
        ...
        # Get vocabulary embedding parameters (for HotFlip only)
        self.word_embeddings = self.ner_model.bert.embeddings.word_embeddings.weight.detach()

        # Get entity label IDs: B-LOC, I-LOC, used to construct the "disguised as location entity" loss
        self.label2id = self.ner_model.config.label2id
        self.id_b_loc = self.label2id['B-LOC']
        self.id_i_loc = self.label2id['I-LOC']

        # Load sentence embedding model (e.g., S-BERT) for embeddings and loss computation
        self.sbert = SentenceTransformer('all-MiniLM-L6-v2', device=device)

# ==========================================
# 4. Strict formula loss computation
# ==========================================
def inject_trigger(texts, trigger_str):
    """
    Randomly insert the trigger at the beginning or end of the text to construct adversarial samples
    Args:
...
def calc_losses_strict(attacker, trigger_str, benign_batch, benign_centers):
    """
    Strictly compute the loss for the trigger according to the formula
    Args:
    ...
    """
    # 1. Construct adversarial sample batch by injecting the current trigger
    ...
    # Mean embedding of the current batch
    adv_centroid = torch.mean(adv_embeds, dim=0, keepdim=True)                  # mu_tau
    # L2 distance from each sample to the mean
    ...
    # Elementwise hinge loss max(0, delta - || e(x_i) - c_n || )
    hinge_matrix = torch.clamp(Config.SAFETY_MARGIN - dists_matrix, min=0)
    # Average to get L_uni
...
# ==========================================
# 5. Compute gradients of the trigger NER loss
#    (to constrain the trigger to look like a location name entity)
# ==========================================
def get_ner_gradients(attacker, current_trigger_ids):
    """
    Get gradients of the loss that makes the trigger recognized as a location entity by NER, w.r.t. token embeddings
    Args:
        attacker: AttackModel object
    ...
    device = attacker.device
    # Use the trigger prefix "Report from " to keep syntax reasonable and get the prefix token ids
    prefix_ids = attacker.tokenizer.encode("Report from ", add_special_tokens=False)
    # Tokenize the trigger and get corresponding embeddings
    ...
    # Clone into an independent variable for differentiation
    ...
    # Build target label sequence: B-LOC (1st) + all I-LOC after
    ...
    trigger_logits = outputs.logits[0, len(prefix_ids):, :]                     # Only take trigger part output
    # Cross-entropy loss (encourage each trigger part to resemble a location entity)
...
# ==========================================
def run_attack():
    # Automatically select device (prefer cuda; fall back to cpu)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    attacker = AttackModel(device)
    
    # 1. Load benign samples and obtain cluster centers
    corpus, benign_centers = load_and_cluster_benign(attacker)
    
    # 2. Initialize the trigger: obtain vocab token ids from initial words
    ...
    # 3. Start iterations (each iteration attempts to optimize one token)
    for step in range(Config.NUM_STEPS):
        ...
        # B. Compute gradients for making the trigger a location entity (guide trigger to look like a place name)
        grads = get_ner_gradients(attacker, current_trigger_ids)
        
        # C. HotFlip search: find the token replacement with the greatest gradient effect
        best_loss = float('inf')
        ...
        for t_idx in range(len(current_trigger_ids)):
            ...
            # Hotflip over the entire vocabulary: find topK tokens with largest directional change (inner product)
            ...
            for cand_id in top_indices:
                ...
                # Filter: keep only valid alphabetic tokens with capital first letter and length > 1
                # (avoid low-quality tokens and defense checks)
                if not (cand_token.isalpha() and cand_token[0].isupper() and len(cand_token) > 1):
                    continue
                    
                # Construct a new candidate trigger (token sequence, string)
                ...
                # D. Evaluate total_loss for the candidate trigger in this iteration
                ...
                if loss_val < best_loss:
                    ...
                    best_str = temp_str
                    # Occasionally print the current loss to observe changes
                    if random.random() < 0.05:
                        print(f"  > Cand: {temp_str} | L_cpt: {metrics['L_cpt']:.4f} | L_uni: {metrics['L_uni']:.4f}")

        # Update trigger after this step
    ...
    # ==========================================
    # 7. Save final optimized result to a json file
    # ==========================================
    ...
    print(f"\n✅ Optimization complete! Results saved: {save_filename}")
    return final_str

# Python entry: running this file directly will trigger attack optimization
